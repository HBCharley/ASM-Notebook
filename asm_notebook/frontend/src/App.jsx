...
export default function App() {
  ...
  async function deleteCompanyWithConfirmation(slug) {
    if (!slug) return;
    if (!confirm(`Delete company '${slug}' and all scans?`)) {
      return;
    }
    await api.deleteCompany(slug);
    const deletedActiveCompany = activeCompany?.slug === slug;
    if (deletedActiveCompany) {
      setSelectedCustomer(ADD_CUSTOMER_OPTION);
      setActiveCompany(null);
      setScans([]);
      setArtifacts(null);
      setArtifactsScanId(null);
    } else if (selectedCustomer === slug) {
      setSelectedCustomer(ADD_CUSTOMER_OPTION);
    }
    await loadCompanies();
  }
              ...
              <>
                {uiMode === "executive" ? (
                  <ExecutiveDashboard
                    ...
                    onDeleteCompany={() =>
                      runWithStatus(() =>
                        deleteCompanyWithConfirmation(activeCompany.slug)
                      )
                    }
                ...
                <div className="card-header">
                  ...
                  <div className="actions">
                    ...
                    <button
                      className="danger"
                      disabled={!canManageActiveCompany}
                      onClick={() => runWithStatus(() => deleteCompanyWithConfirmation(activeCompany.slug))}
                    >
                      Delete company
                    </button>
                  </div>
                        ...
                        <div className="settings-company-group">
                          <div className="row">
                            ...
                            <button
                              ...
                              onClick={() =>
                                runWithStatus(() =>
                                  deleteCompanyWithConfirmation(company.slug)
                                )
                              }
                              title={`Delete ${company.slug}`}
                              aria-label={`Delete ${company.slug}`}
                            >
                              Delete
                            </button>
                          </div>
